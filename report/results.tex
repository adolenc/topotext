...
To visualize persistances of homology groups we used the bar code diagrams and
persistance diagrams. The bar code diagram for a certain homology group $H$ is
...
Persistance diagrams show the same information in the form of a scatter plot.
For each $\gamma$, $pers(\gamma) = [i_{\gamma}, j_{\gamma}]$ in some homology
group we just plot the point $(i_{\gamma}, j_{\gamma})$.

Here are the barcode diagrams and persistance diagrams obtained for each of the
...
maximal filtration, meaning filtration (in particulat parameter $r$) was not
split to some predetermined number of intervals but just left as is and in this
case we were looking at the formations of cech complexes.
